 ...
 **Excel 作为编辑源 → YAML 作为运行时格式**: 领域工程师在 Excel 中维护协议定义和遥测参数表，通过转换脚本生成 YAML 配置文件供引擎加载。
 
 - Excel 是唯一的 source of truth，只在 Excel 中手工编辑
 - YAML 由脚本自动生成，不手工修改
 - git 同时提交 Excel 和生成的 YAML
 ...
 **Excel 编辑源 → YAML 运行时**
 ...
 - 领域工程师使用 Excel 编辑
 - 通过 convert_xlsx_to_yaml.py 生成 YAML
 - git 同时提交 Excel 和 YAML
 
 ### 10.2 工作流
 
 ```
 工程师编辑 Excel → 运行转换脚本 → 生成 YAML → git commit → 引擎加载 YAML
 ```
 ...
 **protocol_template.xlsx**
 ...
 Sheet: FrameConfig | 帧头、校验算法、字节序、字段结构
 Sheet: PhysicalLayer | 波特率、数据位、校验位、停止位
 
 **commands_template.xlsx**
 
 Sheet: ImmediateCmds | 立即指令码
 Sheet: InjectCmds | 固定参数注入指令
 Sheet: ParamEnumValues | 参数枚举值映射
 
 **telemetry_template.xlsx**
 
 Sheet: SlowFrame | 慢帧遥测参数
 Sheet: FastFrame | 快帧遥测参数
 Sheet: EnumValues | 枚举/状态值映射
