...
| changed_to | 从其他值变为目标值（跨帧） | 所有 |
| rising_to | 从 start 单调上升到 end | 数值（需 start_value/end_value/tolerance/within） |
...
Phase 3 已完成开发，实现 5 个核心模块，通过 28 个测试。
...
sequences/*.yaml (.json)
       |
       v
  SequenceLoader          加载 + 校验
       |
       v
  StepScheduler           步骤顺序调度
       |
       +---- CommandTable (指令编码)
       +---- TimelineEngine (锚点 + 时序)
       +---- AssertionEngine (逻辑树评估)
       +---- frame_encoder/frame_decoder
       +---- ProtocolDriver (TCP/RS422)
       |
       v
  TestRunner              编排器 -> 报告
`
...
| 模块 | 文件 | 职责 |
|------|------|------|
| SequenceLoader | sequence_loader.py | YAML/JSON 序列文件加载与校验 |
| AssertionEngine | assertion_engine.py | AND/OR/NOT 递归逻辑树评估 |
| TimelineEngine | timeline_engine.py | 锚点注册/触发、时序队列管理 |
| StepScheduler | step_scheduler.py | 步骤顺序调度、abort/skip/continue |
| TestRunner | test_runner.py | 编排器：load > execute > report |

### 5.3 判据系统

在 Phase 1 设计基础上实现：

tolerance 字段: 所有数值比较器支持容差, abs(actual - expected) <= tolerance

changed_to 操作符: 跨帧状态跳变检测, 需要 prev 帧数据

rising_to 操作符: 单调上升趋势检测, 配合 start_value/end_value/tolerance/within
...
| (Phase 3 引擎) src/engine/*.py | 5 | 测试执行引擎 |
| (Phase 3 序列) sequences/*.yaml | 1 | 测试序列示例 |
